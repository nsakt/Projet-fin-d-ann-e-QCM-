...
def shuffle_enonce_eleves(ListeBase):
    """Mélange l'énoncé d'un élève (une liste de questions) individuels tout en gardant trace de la bonne réponse"""
...
def create_doc_correction(liste_corrections, nom_document):
    """Crée un document Word avec les corrections"""
...
def create_doc_sujets_run(enonce_total, nom_document):
    """Crée des documents Word avec les sujets pour chaque élève"""
    tempstring = ''
    cmpt_sujets = 1
    for elt in enonce_total:
        ...
        for i in range(len(elt)):
            ...
            run.add_break()
            tempstring = ''
        doc_sujet.save(nom_document + 'sujet' + str(cmpt_sujets) + '.docx')
        cmpt_sujets += 1
